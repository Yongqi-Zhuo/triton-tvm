...
@triton.jit
def softmax_kernel(output_ptr, input_ptr, input_row_stride: tl.constexpr, output_row_stride: tl.constexpr, n_cols: tl.constexpr, BLOCK_SIZE: tl.constexpr):
...
def test_softmax(device):
    torch.manual_seed(0)
    x = torch.randn(1823, 781, device=device, requires_grad=False)
    y_triton = softmax(x)
    ...
    print(t_torch)
    print("Benchmarking triton... ", end="")
    t_triton: float = triton.testing.do_bench(lambda: softmax(x))
    print(t_triton)
    print("===")
